            ...
            return (
              <div key={act.id || i} style={{ display: 'flex', gap: 12, padding: '12px 14px', borderBottom: i < 3 ? '1px solid var(--border)' : 'none', alignItems: 'flex-start' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 11, color: 'var(--text1)', fontWeight: 600, lineHeight: 1.4 }}>{act.message || act.description}</div>
                  <div style={{ fontSize: 10, color: 'var(--text3)', marginTop: 2 }}>{timeAgo(act.created_at)}</div>
                </div>
              </div>
            );
          })}
