...
// Explicit origin allowlist — only *.vercel.app and localhost allowed in dev
...
app.use(cors({
  origin: (origin, callback) => {
    // Non-browser requests (Postman, server-to-server) have no Origin header
    ...
    const isAllowed =
      ...
      // In production: only listed Vercel deployments
      (isProd && origin.toLowerCase().includes('vercel.app')) ||
      // In development: any localhost port is fine
      (!isProd && origin.toLowerCase().includes('localhost'));
...
async function startServer() {
  ...
  (async () => {
    try {
      ...
      // FALLBACK: Redis unavailable — start in-process hourly fetch loop
      const { startFallbackWorker } = require('./workers/fallback.worker');
      startFallbackWorker();
    }
  })();
}
